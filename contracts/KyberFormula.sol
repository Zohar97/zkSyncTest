...
////import '../interfaces/IKyberFormula.sol';

contract KyberFormula is IKyberFormula {
   ...
   * @param feePrecision  precision of fee
   *
   * @return amountOut
   */
  function getAmountOut(
    ...
    // validate input
    require(amountIn > 0, 'KyberFormula: INSUFFICIENT_INPUT_AMOUNT');
    require(reserveIn > 0 && reserveOut > 0, 'KyberFormula: INSUFFICIENT_LIQUIDITY');
    uint256 amountInWithFee = amountIn.mul(feePrecision - swapFee);

    // special case for equal weights
    if (tokenWeightIn == tokenWeightOut) {
      return reserveOut.mul(amountInWithFee) / (reserveIn.mul(feePrecision).add(amountInWithFee));
    }

    uint256 result;
    uint8 precision;
    uint256 baseN = reserveIn.mul(feePrecision).add(amountInWithFee);
    (result, precision) = power(baseN, reserveIn.mul(feePrecision), tokenWeightIn, tokenWeightOut);
